...
class DescricaoEleicaoFix2014:
    ...
    def check(self, item):
        return item['year'] == 2014 and item['database'] in ['votos', 'candidatos', 'legendas']
    ...
    def test(self, client):
        for j in [1, 3, 5, 6, 7, 8]:
            cand_df = client.get_candidates(year=2014, job=j, columns=['NOME_CANDIDATO', 'DESCRICAO_ELEICAO'])
            legendas_df = client.get_coalitions(year=2014, job=j, columns=['NOME_PARTIDO', 'DESCRICAO_ELEICAO'])
