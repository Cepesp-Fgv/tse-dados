...
function showQueryResult(data, callback, status, sleep) {
    status = status || "QUEUED";
    sleep = sleep || 1000;

    if (status && (status === "RUNNING" || status === "QUEUED")) {
        delay(sleep).then(function () {
            $.get('/api/consulta/athena/status', {id: data.id})
                .done(function (response) {
                    showQueryResult(data, callback, response.status, sleep * 2);
                })
                .error(showErrorResult);
        });
    } else if (status && status === "SUCCEEDED") {
        downloadBtn.unbind('click').click(function () { download(data.id) });

        $.get('/api/consulta/athena/result', {id: data.id, start: data.start, length: data.length, format: 'json'})
            .done(function (response) {
        ...
        var xhr = {responseJSON: {error: status, status: status === "FAILED" ? 500 : 429}};
        showErrorResult(xhr);
    }
}
